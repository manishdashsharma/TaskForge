...
const SignUp: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    var { name, emailAddress, phoneNumber, password, confirmPassword, consent } = data;
    phoneNumber = phoneNumber.replace('+', '');
    try {
      // Remove confirmPassword from the data sent to the API
      const payload = { name, emailAddress, phoneNumber, password, consent };
      console.log(payload)
      const resp = await SignUpapi(payload); // Assuming SignUpapi is a function that returns a promise
      
      if (resp.success===true) {  // Assuming the API returns a success boolean
        navigate('/'); // Navigate to the home page on success
      } else {
        console.error('Sign-up failed', resp);
      }
    } catch (error) {
              ...
              <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-5'>
                ...
                <div className='flex gap-5 flex-col md:flex-row'>
                  <div className='flex flex-col gap-1'>
                    ...
                    <input type='email' {...register('emailAddress')} placeholder='andreawilliams@gmail.com' className='w-full h-11 focus-visible:outline-none pl-4 rounded-md font-poppins text-[14px] border border-forge-input-border' />
                    {errors.emailAddress && (
                  ...
                  <div className='flex flex-col gap-1'>
                    ...
                    <PhoneInput value={phoneValue || ' '} {...register('phoneNumber')} onChange={handlePhoneChange} type='text' className='w-full bg-white focus:outline-none focus:ring-0 font-poppins text-[14px] h-11 rounded-md px-3 border border-dark-500' />
                    {errors.phoneNumber && (
                ...
                <div className='flex gap-5 flex-col xl:flex-row'>
                  <div className='flex flex-col gap-1'>
                    ...
                    <div className='flex items-center relative'>
                      <input type={showPassword ? 'text' : 'password'} {...register('password')} className='w-full xl:w-[280px] h-11 focus-visible:outline-none px-4 rounded-md font-poppins text-[14px] border border-forge-input-border' />
                      <span className='absolute right-3 cursor-pointer' onClick={() => setShowPassword(!showPassword)}>{showPassword ? <FaEye className='text-forge-darkGreen' /> : <FaEyeSlash className='text-forge-darkGreen' />}</span>
                    </div>
                    {errors.password && (
                  ...
                  <div className='flex flex-col gap-1'>
                    ...
                    <div className='flex items-center relative'>
                      <input type={showConfirmPassword ? 'text' : 'password'} {...register('confirmPassword')} className='w-full xl:w-[280px] h-11 focus-visible:outline-none px-4 rounded-md font-poppins text-[14px] border border-forge-input-border' />
                      <span className='absolute right-3 cursor-pointer' onClick={() => setShowConfirmPassword(!showConfirmPassword)}>{showConfirmPassword ? <FaEye className='text-forge-darkGreen' /> : <FaEyeSlash className='text-forge-darkGreen' />}</span>
                    </div>
                ...
                <div className='flex items-center gap-2'>
                  <input type='checkbox' {...register('consent')} className='h-5 w-5 border-gray-300 rounded-md text-forge-darkGreen focus:ring-forge-darkGreen' />
                  <Label htmlFor='consent' className='font-poppins text-[15px] font-normal'>I agree to the <Link to="/terms" className="text-forge-darkGreen">terms and conditions</Link></Label>
                </div>
                {errors.consent && (
                  <p className='font-poppins font-normal text-yellow-600 text-[13px] flex gap-1 items-center'><RiErrorWarningFill className='text-yellow-600 size-4' /> {errors.consent.message}</p>
                )}

                <div className='mt-4'>
                  <Button type='submit' disabled={!isValid} className='bg-forge-darkGreen text-white py-3 rounded-md w-full font-poppins'>Sign Up</Button>
                </div>
              </form>
            </div>
            <div className='mt-4'>
              <p className='font-poppins text-[15px] font-normal'>Already have an account? <Link to="/login" className='text-forge-darkGreen'>Log in here.</Link></p>
            </div>
          </div>
        </div>
